...
def bg_likelihood(x, data, nominal_atm_bg, masks):
    nominal_sig = [np.zeros(d.shape) for d in nominal_atm_bg]
    out = sig_likelihood([0, x[0]], data, nominal_sig, nominal_atm_bg, masks)
    return out
...
def sig_likelihood(
    ...
    out = []
    for d, sig, bg, m in zip(data, nominal_sig, nominal_atm_bg, masks):
        out.append(-poisson_loglikelihood(d[m], x[0] * sig[m] + x[1] * bg[m]))
    return out
...
def run_trials(
    ...
    true_model = [
        norm * sig + bg for sig, bg in zip(nominal_sig, nominal_atm_bg)
    ]
    ...
    for idx in tqdm(range(nrealizations)):
        ...
        g = lambda x: np.sum([
            x.sum() for x in bg_likelihood(x, data, nominal_atm_bg, masks)
        ])
        f = lambda x: np.sum([
            x.sum() for x in sig_likelihood(x, data, nominal_sig, nominal_atm_bg, masks)
        ])
        fitg = minimize(g, [1], bounds=[(0.99, 1.01)], tol=1e-20)
            
        fitf = minimize(f, [2, fitg.x[0]], bounds=[(0, 100), (0.99, 1.01)], tol=1e-20)
        ...
        we = [
            (x-y).sum() for x, y in zip(
                sig_likelihood(fitf.x, data, nominal_sig, nominal_atm_bg, masks),
                bg_likelihood(fitg.x, data, nominal_atm_bg, masks),
            )
        ]
        delta = -2 * np.sum(we)
        res[idx, 0] = fitf.x[0]
        ...
        if idx % 100 ==99:
            ...
            print(np.quantile(res[:idx, 3], [0.5, 0.9]))
    return res
...
def main(
    ...
    np.random.seed(seed)
    res = run_trials(nominal_sig, nominal_atm_bg, norm, n)
