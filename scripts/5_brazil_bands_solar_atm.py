...
def bg_likelihood(x, data, nominal_atm_bg, masks):
    ...
    out = sig_likelihood([0, x], data, nominal_sig, nominal_atm_bg, masks)
    return out
...
def run_trials(
    ...
    masks = [iso_bg > 0 for iso_bg in nominal_atm_bg]
    true_model = nominal_atm_bg
    ...
    for idx in tqdm(range(nrealizations)):
        ...
        llh0 = [x.sum() for x in bg_likelihood(fitg.x, data, nominal_atm_bg, masks)]
        llh1 = lambda lx: [
...
def main(
    ...
    with h5.File(trials_file, "r") as h5f:
        for key in h5f.keys():
            ...
            bg_ts_90 = np.quantile(val[:, 3], 0.9)
    print(bg_ts_90)
    nominal_atm_bg = []
    for f in bgfiles:
    ...
    res = run_trials(nominal_sig, nominal_atm_bg, nrealizations, bg_ts_90)
